...
### Operator-Specific Domain Hints

| Domain | Patterns | Preferred Agent(s) |
| ------ | -------- | ------------------ |
| **Healthcare** | ris, pacs, his, lis, clinico, clinical, hl7, fhir, dicom | `orchestrator` or `backend-specialist` depending on scope |
| **Automation Ops** | n8n, workflow, automacao, automation, atendimento, triagem | `orchestrator` or `backend-specialist` depending on scope |
| **Observability** | logs, metricas, metrics, monitoramento, monitoring, alerta, uptime | `devops-engineer` |
| **SaaS B2B** | multi-tenant, tenant, rbac, billing, saas | `orchestrator` or `backend-specialist` depending on scope |
| **Integration Reliability** | webhook, etl, sync, retry, fallback, reconciliacao, reconciliation | `backend-specialist` |
...
Also auto-invoke `orchestrator` when requests combine any of these operator-specific domains:

- healthcare + integration
- SaaS + RBAC or tenant isolation
- automation + AI + external systems
...
- Auto-routing does not bypass clarification when critical information is missing
- If the task is already clear enough, route and proceed
- Then ask follow-up questions only if execution reveals a real ambiguity
...
## Operator Profile Alignment

When the workspace has an `operator-profile/` layer:

1. Prefer the operator's default stack unless the user says otherwise.
2. Prefer practical, integration-friendly architecture over trend-driven choices.
3. Escalate to `orchestrator` sooner for healthcare, multi-tenant, security, observability, and automation tasks.
4. Favor Node + PostgreSQL + Docker style recommendations when technically appropriate.
